...
impl<'a> Map<'a> {
    ...
    pub fn render(&self, canvas: &mut WindowCanvas, camera_rect: &Rect) {
        if let tiled::LayerData::Finite(tiles) = &self.layer.tiles {
            let (tile_left, tile_top) = self.point_to_tile(camera_rect.x, camera_rect.y);
            let (tile_right, tile_bottom) =
                self.point_to_tile(camera_rect.x + camera_rect.w, camera_rect.y + camera_rect.h);
            ...
            let tile_start_x = camera_rect.x - tile_left * self.tile_width as i32;
            let tile_start_y = camera_rect.y - tile_top * self.tile_height as i32;
                    ...
                    if gid != 0 {
                        ...
                        canvas
                            .copy_ex(
                                ...
                                Some(rect),
                                Some(Rect::new(
                                    (x - tile_left) as i32 * self.tile_width as i32 - tile_start_x,
                                    (y - tile_top) as i32 * self.tile_height as i32 - tile_start_y,
                                    self.tile_width,
                                    self.tile_height,
                                )),
                                0.0,
